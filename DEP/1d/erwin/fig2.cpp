...
double TemporalMean(DEPmodel1d & density_production, Params& params){

	//double* temporal_density_B = new double[params.T_max]{};
	//DEPmodel1d density_production(params.L,total_number,params.z,params.T_max);
	density_production.FulcoAlgorithm(params.T_max);
	
	double density = 0;
	for(int t=0;t<400;t++) density += density_production.density_B[params.T_max -1 - t]/400;

	//delete []temporal_density_B;
	return density;	
}

double SampleMean(Params& params, int run_by_onecore, int total_number){
	double density_B = 0;
	for(int i=0; i<run_by_onecore;i++){
		DEPmodel1d density_production(params.L,total_number,params.z,params.T_max, params.init_dist);
		density_B += TemporalMean(density_production, params)/run_by_onecore;
	}
	return density_B;
}
...
int main(int argc, char* argv[]){
	...
	int parallelization = 50000, run_by_onecore = parallelization/NUM_WORKERS;
	int L = atoi(argv[1]), z =2,T_max = 100000;//1e+7
	int init_total_number= atof(argv[2])*L, final_total_number = 0.4*L, steps = 80;
	...
	double* density_Bsite = new double[params.T_max]{};
	double* P_survival = new double[params.T_max]{};
	double* radius = new double[params.T_max]{};

	double* mpi_density_B = new double[params.T_max]{};
	double* mpi_density_Bsite = new double[params.T_max]{};
	double* mpi_P_survival = new double[params.T_max]{};
	double* mpi_radius = new double[params.T_max]{};
	...
	MPI::COMM_WORLD.Reduce(density_B, mpi_density_B ,T_max,MPI::DOUBLE,MPI::SUM,0);
	MPI::COMM_WORLD.Reduce(density_Bsite, mpi_density_Bsite ,T_max,MPI::DOUBLE,MPI::SUM,0);
	MPI::COMM_WORLD.Reduce(P_survival, mpi_P_survival ,T_max,MPI::DOUBLE,MPI::SUM,0);
	MPI::COMM_WORLD.Reduce(radius, mpi_radius ,T_max,MPI::DOUBLE,MPI::SUM,0);

	if(WORKER==0){
		for(int t=0;t<T_max;t++){
			cout<<t<<"\t"<<mpi_density_B[t]/NUM_WORKERS<<"\t"<<mpi_P_survival[t]/NUM_WORKERS<<"\t"<<mpi_density_Bsite[t]/NUM_WORKERS<<"\t"<<mpi_radius[t]/NUM_WORKERS<<endl;
