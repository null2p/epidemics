...
int main(int argc, char* argv[]) {
    ...
    int * n;
    float W_A=0,W_B=0,W_R=0,W_I=0,W_T=0;
    double dT, p, q,t,D_A = 0.5, D_B = 0.5;
    vector<int> A(L);
    //vector<int> A_arg;
    vector<int> B(L);
    //vector<int> B_arg;
    ...
    //initialization, half of them are 'A' and others are '0' 
    ...
    W_A = A_sum*D_A; W_B = B_sum*D_B; W_R = r*B_sum;
    W_I = std::inner_product(A.begin(), A.end(), B.begin(), 0);
    W_T= W_A +W_B + W_R + W_I;
    ...
    while(steps<T){
        ...
        //cout<<endl;
        if((W_A+W_B)/W_T>mt_uniform(generator))  Diffusion(WORKER, D_A, D_B,L, A, B,steps);
        else  Reaction(WORKER, L, A, B,steps);
